...
st.markdown("<h2 style='text-align: center; color: black;'>Inicio de Sesión</h2>", unsafe_allow_html=True)
...
if user_input and pass_input:
    if user_input == username and pass_input == password:
            ...
            # Ask the user about the type of tax
                if tax_type in ["Real Estate", "Business Personal Property"]:
                    ...
                    if is_applebees == "Yes":
                        ...
                        if use_same_bp == "Yes":
                            # Ask the user to type the value for Store# to find the Vendor ID
                            store_number = st.text_input("Enter the **Store#**:")

                            # Find the Vendor ID based on the input
                            ...
                            if not vendor_id_row.empty:
                                ...
                                df["BP"] = vendor_id
                                st.success(f"Store# {store_number} is valid. Vendor ID: {vendor_id} added")

                            else:
                                st.error(f"Store# {store_number} is not valid. Please check the Store#.")
                        else:
                            # Ask the user to type the value for each record
                            df["BP"] = ""
                            for index, row in df.iterrows():
                                st.write(f"Please enter the value for the following record:")
                                st.write(f"Store: {row['Store #']} of {row['BRAND']}")
                                st.write(f"Site: {row['Site']}, Company: {row['Company']}")
                                store_number = st.text_input(f"Enter the Store# for record {index + 1}:", key=f"store_number_{index}")
                                ...
                                if not vendor_id_row.empty:
                                    ...
                                    st.success(f"Store# {store_number} is valid for record {index + 1}. Vendor ID: {vendor_id} added.")

                                else:
                                    st.error(f"Store# {store_number} is not valid for record {index + 1}. Please check the Store#.")
                    else:
                        # Ask the user if they want to use the same BP (Vendor ID) for all records
                        use_same_bp = st.radio("Do you want to use the same BP (Vendor ID) for all records?", ("Yes", "No"))
        ...
        #--------------------------------------------------------------------------------
                ...
                comment_columns = ["Parcel", "Parcel ", "PARCEL", "Account", "Account "]
        ...
        #--------------------------------------------------------------------------------
                ...
                output_path = "output.xlsx"  # Output path
